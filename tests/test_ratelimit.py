...
from glimpse_markets.ratelimit import AsyncRateLimiter, RateLimiter
...
async def test_async_acquire_does_not_block_under_budget() -> None:
    limiter = AsyncRateLimiter(max_requests=3, window_seconds=60.0)
...
async def test_async_acquire_blocks_once_budget_exhausted() -> None:
    limiter = AsyncRateLimiter(max_requests=2, window_seconds=10.0)
    fake_now = [1000.0]
    ...
    with (
        patch("time.monotonic", side_effect=fake_monotonic),
        patch("asyncio.sleep", side_effect=fake_sleep) as sleep_mock,
    ):
        await limiter.acquire()  # t=1000
        await limiter.acquire()  # t=1000, budget now full
        await limiter.acquire()  # must wait until t=1010 before the 3rd request
...
async def test_async_expired_timestamps_are_evicted() -> None:
    limiter = AsyncRateLimiter(max_requests=1, window_seconds=5.0)
    fake_now = [0.0]

    with patch("time.monotonic", side_effect=lambda: fake_now[0]):
        await limiter.acquire()  # t=0, fills the single slot
        fake_now[0] = 10.0  # well past the 5s window
        with patch("asyncio.sleep") as sleep_mock:
            await limiter.acquire() 
        sleep_mock.assert_not_called()
